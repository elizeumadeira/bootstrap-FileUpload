...
;(function ($, document, window, undefined) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (instance[el].options.showThumb === true) {
					...
					if (options.multiUpload === false) {
						...
						row = '<tr class="fileupload-previewrow no-thumb row" id="' + fileName + '"><td class="col-lg-10">' + file.name + '</td><td class="col-lg-2"><button class="btn btn-danger fileupload-remove" value="' + fileName + '"><i class="glyphicon glyphicon-ban-circle"></i>&nbsp;<span>'+instance[el].options.language.btn_removefile+'</span></button></td></tr>';
					}
				}
			...
			if (instance[el].options.multiUpload === false) {
					...
					$.ajax({
						...
						success: function(data, status, xhr) {
							...
							if (typeof instance[el].options.onUploadSuccess === 'function') {
								...
								instance[el].options.onUploadSuccess(data, status, xhr, this);
							}
						},
				...
				$.ajax({
					...
					success: function (data, status, xhr) {
						...
						if (typeof instance[el].options.onUploadSuccess === 'function') {
							// instance[el].options.onUploadSuccess.call(this);
							instance[el].options.onUploadSuccess.call(data, status, xhr, this);
						}
					},
					error: function (xhr, status, err) {
						instance[el].overallProgressBar.find(".progress-bar-striped").attr("aria-valuenow", 0).css("width", "0%");
	...
	function formStructure (el) {
		...
		instance[el].btnAdd = $('<div class="btn btn-success fileupload-add"><input type="file" ' + (instance[el].options.multiFile === true ? 'multiple="multiple"' : void 0) + 'multiple /><i class="glyphicon glyphicon-plus"></i>&nbsp;'+instance[el].options.language.btn_addfile+'&hellip;</div>');
		instance[el].btnReset = $('<button type="reset" class="btn btn-primary fileupload-reset"><i class="glyphicon glyphicon-repeat"></i>&nbsp;'+instance[el].options.language.btn_addmorefiles+'&hellip;</button>');
		instance[el].btnStart = $('<button class="btn btn-warning fileupload-start"><i class="glyphicon glyphicon-upload"></i>&nbsp;<span>'+instance[el].options.language.btn_startupload+'</span></button>');
		instance[el].btnCancel = $('<button type="reset" class="btn btn-danger fileupload-cancel"><i class="glyphicon glyphicon-ban-circle"></i>&nbsp;<span>'+instance[el].options.language.btn_cancelupload+'</span></button>');
		...
		instance[el].overallStatus = $('<div class="row fileupload-overall-status"><div class="col-lg-12"><div class="alert alert-success"><strong>'+instance[el].options.language.message_uploadedsuccessfully+'</strong></div><div class="alert alert-danger"></div></div></div>');
	...
	function fallbackFormStructure (el) {
		...
		instance[el].btnAdd = $('<div class="input-group"><span class="input-group-btn"><span class="btn btn-success fileupload-fallback-add"><i class="glyphicon glyphicon-plus"></i>&nbsp;' + instance[el].options.language.btn_addfiles + '&hellip; <input type="file" name="' + instance[el].options.inputName + '" ' + (instance[el].options.multiFile === true ? 'multiple="multiple"' : void 0) + '></span></span><input type="text" class="form-control" readonly></div>');
		instance[el].btnStart = $('<div class="form-group"><button type="submit" class="btn btn-warning fileupload-fallback-start"><i class="glyphicon glyphicon-upload"></i>&nbsp;<span>' + instance[el].options.language.btn_startupload + '</span></button><button type="reset" class="btn btn-primary fileupload-fallback-reset"><i class="glyphicon glyphicon-repeat"></i>&nbsp;Reset</button></div>');
	...
	$.fn.bootstrapFileUpload.defaults = {
		...
		language: {
			...
			btn_addfiles: "Add Files",
			btn_startupload: "Start Upload",
			btn_cancelupload: "Cancel Upload",
			btn_addmorefiles: "Add More Files",
			btn_removefile: "Remove File",
			message_uploadedsuccessfully: "Uploaded Successfully!"
		},
		debug: true,
		onInit: function () {},
